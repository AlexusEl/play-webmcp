...
examples/java/target/universal/stage/bin/play-webmcp-java-example \
  -J-Xms64m -J-Xmx384m -Dhttp.port=19001 -Dpidfile.path="$task_test_dir/java.pid" \
  >"$task_test_dir/java.log" 2>&1 &
java_pid=$!
examples/scala/target/universal/stage/bin/play-webmcp-scala-example \
  -J-Xms64m -J-Xmx384m -Dhttp.port=19002 -Dpidfile.path="$task_test_dir/scala.pid" \
...
for port in 19001 19002; do
  ...
  for attempt in $(seq 1 90); do
    if curl --fail --silent "http://localhost:$port/" >/dev/null; then ready=true; break; fi
    sleep 1
  done
  if [[ "$ready" != true ]]; then
  ...
  BASE_URL="http://localhost:$port" node tests/browser.mjs
done
